...
export type ScriptStep =
  ...
  | {
      kind: "tools";
      // Optional leading text before the tool calls.
      text?: string;
...
/** A model whose doStream rejects — used to test error handling. */
export function throwingModel(message = "boom"): MockLanguageModelV2 {
  return new MockLanguageModelV2({
    doStream: async () => {
